...
# -*- coding: utf-8 -*-
"""
EigenPlaces-style Classifier (CLI Version)
...
Projection + Whitening:
  - 512 -> proj_dim (Linear + BN)
  - Learnable whitening (Linear proj_dim->proj_dim, init orthogonal)
  - Ortho regularization: ||W^T W - I||_F^2

Head:
  - CosFace / AM-Softmax style cosine margin head

Loss:
  - CrossEntropy(logits, y) + lambda_ortho * ortho_loss
...
# ==============================
def parse_args():
    p = argparse.ArgumentParser("EigenPlaces-style Classifier")
    ...
    p.add_argument("--freeze_backbone_epochs", type=int, default=0,
                   help="前 K 个 epoch 冻结骨干网络，只训练投影/白化/头")
    ...
    p.add_argument("--cos_m", type=float, default=0.35,
                   help="CosFace margin m")

    # Whitening 超参
    p.add_argument("--proj_dim", type=int, default=256,
                   help="投影维度 (512 -> proj_dim)")
    p.add_argument("--lambda_ortho", type=float, default=1e-3,
                   help="正交约束权重")
    ...
    p.add_argument("--save_model", type=str, default="eigenplaces_best.pt",
                   help="最佳模型保存路径")
    p.add_argument("--save_cm", type=str, default="",
                   help="混淆矩阵保存为图片；为空则直接展示")
...
class CosMarginProduct(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        cos_theta = torch.matmul(x, W.t())        # [B,C]
...
# 3) 模型定义
# ==============================
class EigenPlacesClassifier(nn.Module):
    def __init__(self, num_classes, proj_dim=256,
                 s=30.0, m=0.35, pretrained=False):
        super().__init__()
        ...
        self.feat_dim_in = 512

        # 512 -> proj_dim
        self.proj = nn.Linear(self.feat_dim_in, proj_dim, bias=False)
        self.bn_proj = nn.BatchNorm1d(proj_dim)

        # Learnable Whitening W (proj_dim x proj_dim)
        self.whiten = nn.Linear(proj_dim, proj_dim, bias=False)
        nn.init.orthogonal_(self.whiten.weight)
        self.bn_white = nn.BatchNorm1d(proj_dim)

        # CosFace / AM-Softmax 头
...
def orthogonality_loss(W: torch.Tensor) -> torch.Tensor:
    ...
    I = torch.eye(d, device=WT_W.device, dtype=WT_W.dtype)
...
# 4) 训练 & 评测逻辑
# ==============================
def set_backbone_trainable(model: EigenPlacesClassifier, flag: bool):
    ...
    for imgs, labels in loader:
        ...
        with torch.set_grad_enabled(train):
            if scaler is not None and scaler.is_enabled():
                with autocast():
                    # 训练：带 margin；验证：不带 margin
                    logits = model(imgs, labels if train else None)
                    ce = criterion(logits, labels)
                    ortho = orthogonality_loss(model.whiten.weight)
                    loss = ce + lambda_ortho * ortho
            else:
                logits = model(imgs, labels if train else None)
                ce = criterion(logits, labels)
                ortho = orthogonality_loss(model.whiten.weight)
...
def main(args):
    ...
    with torch.no_grad():
        for imgs, labels in test_loader:
            imgs = imgs.to(device, non_blocking=True)
            # 验证/测试时 labels=None -> 无 margin logits
            logits = model(imgs, labels=None)
    ...
    if args.save_cm:
        ...
        plt.title("Confusion Matrix (EigenPlaces-style Classifier)")
